  ...
  Mutation: {
    EditPhotoRN: protectResolver(
      async (
        ...
        console.log(id)
        const oldPhoto = await client.photoRN.findFirst({
          where: {
            id,
        ...
        if (file) {
          fileUrl = await uploadToS3(file, loggedInUser.id, 'uploadPhotoRN')
        }
        if (file2) {
